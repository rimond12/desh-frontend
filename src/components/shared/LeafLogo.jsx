...
// Big colored leaf — accepts colorCode (hex) directly from eval rules
// level name shown below; colorCode drives all colors
export function ColoredLeaf({ level, colorCode, size = 90 }) {
  const base   = colorCode || '#94A3B8';
  const dark   = darkenHex(base, 0.32);
  const shadow = colorCode
    ? `${base}55`
    : 'rgba(0,0,0,0.10)';
  // unique gradient id based on color to avoid SVG id collision
