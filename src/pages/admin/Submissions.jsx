...
export default function Submissions() {
  ...
  const fetchSubmissions = useCallback((overrides = {}) => {
    ...
    axiosSecure.get(`/submissions?${params.toString()}`)
      .then(res => {
        ...
        // ── Count comparison log ──
        console.log('[Submissions] ─── Count Comparison ───');
        console.log(`  MongoDB total (all matching):  ${pg?.total ?? '?'}`);
        console.log(`  API returned (this page):      ${received.length}`);
        console.log(`  Page: ${pg?.page} / ${pg?.totalPages}  |  limit: ${l}`);
  ...
  console.log(`[Submissions] In state=${projects.length} | after levelFilter="${levelFilter}"=${filtered.length} | rendered in table`);
