...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      const colors = {
        'Green Leaf': { color: '#22C55E', from: '#16520A', to: '#22C55E' },
  ...
  return (
    <Layout isAdmin>
      <div className="mb-8 fade-in-up">
        <p className="text-sm" style={{ color: 'rgba(232,245,233,0.4)' }}>Admin Panel</p>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        {[
          { label: 'Total Users', value: stats.users, icon: '◉', color: '#22C55E' },
          { label: 'Projects', value: stats.projects, icon: '◫', color: '#4ADE80' },
          ...
          <div key={i} className="stat-card">
            ...
            <p className="text-xs mt-0.5" style={{ color: 'rgba(232,245,233,0.35)' }}>{s.label}</p>
          </div>
        ))}
      </div>
      ...
      <div className="grid xl:grid-cols-3 gap-6 mb-6">
        ...
        <div className="xl:col-span-2 glass-card overflow-hidden">
          <div className="flex items-center justify-between px-6 py-4 border-b" style={{ borderColor: 'rgba(34,197,94,0.1)' }}>
            ...
            <Link to="/admin/submissions" className="text-xs font-semibold" style={{ color: '#4ADE80' }}>View all →</Link>
          </div>
          {loading ? (
              ...
              <tbody>
                {submissions.map(s => (
                  <tr key={s._id}>
                    ...
                    <td className="font-bold text-white text-sm">{s.scorePercent || 0}%</td>
                    <td className="text-xs">{new Date(s.updatedAt).toLocaleDateString()}</td>
                  </tr>
                ))}
                {submissions.length === 0 && (
                  <tr><td colSpan={5} className="text-center py-8" style={{ color: 'rgba(232,245,233,0.3)' }}>No submissions yet</td></tr>
                )}
              </tbody>
            </table>
          )}
        ...
        <div className="space-y-5">
          ...
          <div className="glass-card p-5">
            <h3 className="font-bold text-white mb-4">🍃 Leaf Distribution</h3>
            {leafDist.length === 0 ? (
              <p className="text-xs text-center py-4" style={{ color: 'rgba(232,245,233,0.3)' }}>No data yet</p>
            ) : (
                    ...
                    <div key={l.level}>
                      <div className="flex items-center justify-between mb-1">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-xs font-semibold text-white">{l.level}</span>
                        </div>
                        <span className="text-xs" style={{ color: 'rgba(232,245,233,0.4)' }}>{l.count} ({pct}%)</span>
                      </div>
            ...
            <div className="space-y-3">
              {activity.slice(0, 5).map((a, i) => (
                <div key={i} className="flex gap-3 items-start">
                  <span className={`text-xs px-1.5 py-0.5 rounded font-semibold flex-shrink-0 ${a.actorType === 'admin' ? 'bg-orange-500/15 text-orange-400' : 'bg-green-500/10 text-green-400'
                    }`}>{a.actorType}</span>
                  <div>
                    <p className="text-xs leading-relaxed" style={{ color: 'rgba(232,245,233,0.7)' }}>{a.description}</p>
                    <p className="text-xs mt-0.5" style={{ color: 'rgba(232,245,233,0.25)' }}>
                      {new Date(a.createdAt).toLocaleString()}
                    </p>
                  </div>
                </div>
              ))}
              {activity.length === 0 && <p className="text-xs text-center" style={{ color: 'rgba(232,245,233,0.3)' }}>No activity yet</p>}
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          { label: 'Submissions', icon: '◫', to: '/admin/submissions', color: '#22C55E' },
          { label: 'Tabs & Modules', icon: '◈', to: '/admin/tabs', color: '#F8A514' },
          { label: 'Leaf Levels', icon: '⬡', to: '/admin/leaf-levels', color: '#4ADE80' },
